...
const handleClick = (event: Event): void => {
  ...
  // First emit validate to check form
  emit('validate')
  
  // If there are errors, don't submit
  if (props.hasErrors) {
    // Focus button to show error state
 ...
 */
const submit = (): void => {
  // Emit submit after animation completes
  setTimeout(() => {
    emit('submit')
  }, 2000)
}
...
<template>
  ...
  >
    ...
    <div class="state state--error" v-if="hasErrors">
      <div class="icon">
        <svg
          stroke="currentColor"
          stroke-width="0.5px"
...
<style scoped>
/* Adapted for Lecabe Brand */ 
.button {
  --primary: #4f52ff; /* Lecabe primary */
  --success: #10b981; /* Green for success */
  --error: #ef4444; /* Red for error */
  --neutral-1: #4f52ff; /* Lecabe primary blue */
  --neutral-2: #3c3fbb; /* Lecabe dark primary */
  --radius: 12px;
...
.button:hover::after {
  ...
  box-shadow: inset 0 -1px 3px 0 rgba(60, 63, 187, 0.6);
}
...
.button:focus .state--default .icon {
  transform: translate(-50%, -50%) rotate(0) scale(1.25);
}
@keyframes takeOff {
  0% {
    opacity: 1;
...
.button.has-errors:focus {
  background: var(--error);
}
.button.has-errors:focus:after {
  background: var(--error);
}
...
.button.has-errors:focus .state--default {
  position: absolute;
}
.button.has-errors:focus .state--sent {
  display: none;
}
.button.has-errors:focus .state--error {
  display: flex;
}
.button.has-errors:focus .state--error span {
  opacity: 0;
  animation: slideDown 0.8s ease forwards calc(var(--i) * 0.2s);
}
.button.has-errors:focus .state--error .icon svg {
  opacity: 0;
  animation: appear 1.2s ease forwards 0.8s;
}
@keyframes appear {
